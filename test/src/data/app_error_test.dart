...
void main() {
  test('AppError Test', () async {
    ...
    expect(
        AppError(
          DioError(type: DioErrorType.RESPONSE, response: Response(statusCode: 400)),
        ).type,
        equals(AppErrorType.badRequest));

    expect(
        AppError(
          DioError(type: DioErrorType.RESPONSE, response: Response(statusCode: 401)),
        ).type,
        equals(AppErrorType.unauthorized));

    expect(
        AppError(
          DioError(type: DioErrorType.RESPONSE, response: Response(statusCode: 500)),
        ).type,
    ...
    expect(
        AppError(
          DioError(error: const SocketException('Failed host lookup: wasabeef.jp'), type: DioErrorType.DEFAULT),
        ).type,
        equals(AppErrorType.network));
