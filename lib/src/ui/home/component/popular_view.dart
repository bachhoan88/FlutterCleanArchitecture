    ...
    return ContainerWithLoading(
      child: HookBuilder(
        builder: (context) {
          ...
          final movies = useProvider(homeViewModelProvider.select((value) => value.popularMovies));
          final snapshot = useFuture(
            useMemoized(() {
              return context.read(loadingStateProvider).whileLoading(viewModel.fetchPopular);
            }, [movies.toString()]),
          );
          ...
          return movies.when(success: (data) {
            ...
            return ErrorPage(message: e.message ?? '', retry: () async => viewModel.fetchPopular);
          });
        },
      ),
    );
